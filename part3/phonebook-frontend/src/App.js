...
const App = () => {
  ...
  const addPerson = (event) => {
    ...
    if (names.find(name => name === newName)) {
      ...
      peopleService
        ...
        // give error if update did not work e.g., number format incorrect
        .catch(error => {
          setError(
    ...
    peopleService
      ...
      // if name too short, send error to screen
      .catch(error => {
  ...
  const removePerson = id => {
    ...
    if (window.confirm(`Delete ${persons[idx].name}?`)){
      peopleService
        .remove(id)
        setPersons(persons.filter(p => p.id !== id))
        setNotification(
          `'${persons[idx].name}' Removed`
        )}
