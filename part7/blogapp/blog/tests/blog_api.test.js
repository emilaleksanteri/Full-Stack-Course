...
describe('blog edits', () => {
  ...
  test('add a comment to a blog', async () => {
    const allBlogs = await helper.allBlogs()
    const blogToComment = allBlogs[0]

    const commentToPost = {
      comment: 'great blog',
      blog: blogToComment.id
    }
    ...
    const blogsAfter = await helper.allBlogs()
    const blogToCompare = blogsAfter[0]

    expect(blogToCompare.comments).toHaveLength(1)
  })
})
