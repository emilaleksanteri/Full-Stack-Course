...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      ...
      if (!args.genre && args.author) {
        return books.filter((b) => b.author === args.author) // return books w matching author
      }

      if (!args.author && args.genre) {
        return books.filter((b) => b.genres.includes(args.genre)) // books w matching genre
      }
      // returns books of auth w specific genre
  ...
  Author: {
    bookCount: (obj) => {
      const booksOfAuthor = books.filter((b) => b.author === obj.name)
